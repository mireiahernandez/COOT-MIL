...
for j, movie in enumerate(movies):
    ...
    for i in range(num_align):
        if movie != 'The.Road' or i != 127: # missing data
            ...
            out_name = '~/public_html/tmp2/{}_{}.mp4'.format(movie, i)
            #print(out_name)
            ...
            lines.append('<tbody>')
            lines.append('<tr>')
            lines.append('<div> <video preload="none" width="500px"; controls><source src="{}"></video></div><div>{}:{}</div>'.format(src_name, convertTime(clip_start),convertTime(clip_end)))
            lines.append('</tr><tr>')
            lines.append('<div style=color:black>Sentence {}: {}</div>'.format(id_sentence, sentence))
...
with open('{}_corr.html'.format(movie), 'w') as out:
    out.writelines(ll)
    
os.system('mv {}_corr.html ~/public_html/{}_corr.html'.format(movie, movie))
