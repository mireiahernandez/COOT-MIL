...
labeled_sents = json.load(open(cootmil_path + 'data/bksnmovies/labeled_sents.json', 'r'))
...
for i, movie in enumerate(movies):
    ...
    for scene_key in seen_scenes:
        ...
        for anno_num, anno in enumerate(annos):
            ...
            if len(neigh_sents) == 0: ipdb.set_trace()
            ...
            # Define the bag of positive candidates
            bag = {'anno_num': anno_num,'positive_shots':[], 'positive_sentences':[]}
            
            # Add sentences to text_data and to bag
            for neigh_id in neigh_sents:
                text_data[scene_key][sent_id] = book_file['book']['sentences'].item()['sentence'][neigh_id][0][0]
                bag['positive_sentences'].append(sent_id)
                sent_id += 1
            
            # Add shots to vid data and to bag
            for neigh_id in neigh_shots:
                vid_data[scene_key][shot_id] = [shot_df['rel_start_time'][neigh_id], shot_df['rel_end_time'][neigh_id]]
                bag['positive_shots'].append(shot_id)
                shot_id += 1
            
            # Add bag to pos_data
            pos_data[scene_key].append(bag)
...
json.dump(coot2, open(cootmil_path + 'annotations/bksnmovies/bknmovies_v0_split_{}_nonempty.json'.format(dataset_split), 'w'))
print('Saved coot2 at ../annotations/bksnmovies/bknmovies_v0_split_{}_nonempty.json'.format(dataset_split))
json.dump(coot2, open(coot_path + 'annotations/bksnmovies/bknmovies_v0_split_{}_nonempty.json'.format(dataset_split), 'w'))
print('Saved coot2 at ../../coot-videotext/annotations/bksnmovies/bknmovies_v0_split_{}_nonempty.json'.format(dataset_split))

json.dump(text_data, open(cootmil_path + 'data/bksnmovies/text_data.json', 'w'))
json.dump(text_data, open(cootmil_path + 'annotations/bksnmovies/text_data.json', 'w'))
json.dump(vid_data, open(cootmil_path + 'annotations/bksnmovies/vid_data.json', 'w'))
json.dump(vid_data, open(cootmil_path + 'data/bksnmovies/vid_data.json', 'w'))
json.dump(pos_data, open(cootmil_path + 'annotations/bksnmovies/pos_data.json', 'w'))
