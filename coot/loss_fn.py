...
class MILContrastiveLoss(nn.Module):
    """
    Multi-instance NCE Contrastive Loss between 2 groups of embeddings
    """
    def __init__(self, margin: float, max_violation: bool = False, norm: bool = True, use_cuda: bool = True):
        super().__init__()
        self.margin = margin
    ...
    def forward(self, im, se, pos, neg):
        ...
        Args:
            im: Visual embeddings (batch, embed_dim)
            s: Text embeddings (batch, embed_dim)
            pos_sent_clip: positive pairs List[List[th.Tensor]] len #batch, len #bags, shape (2, #pairs)
