...
class RetrievalDataset(th_data.Dataset):
    ...
    def __init__(self, cfg: RetrievalDatasetConfig, path_data: Union[str, Path], *,
        ...
        for key, data_key in zip(self.keys, self.data_keys):
            ...
            for seg in self.meta[key]["vid_segments"]:
    ...
    def __getitem__(self, item: int) -> RetrievalDataPointTuple:
        ...
        # load number of clips
        clip_num = len(vid_dict["vid_segments"])
        ...
        clip_feat_list = []
        clip_feat_len_list = []
        for i, seg in enumerate(vid_dict["vid_segments"]):
        ...
        # ---------- load text as string ----------
        seg_narrations = []
        for seg in vid_dict["text_segments"]:
            seg_narr = seg["text"]
            if seg_narr is None:
                seg_narr = "undefined"
        ...
        # Load number of sentences
        sent_num = len(seg_narrations)
