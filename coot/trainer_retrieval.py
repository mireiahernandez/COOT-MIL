...
from coot.loss_fn import ContrastiveLoss, MILContrastiveLoss, CycleConsistencyLoss, LossesConst
...
class RetrievalTrainer(trainer_base.BaseTrainer):
    ...
    def __init__(
        ...
        self.loss_MILcontr = MILContrastiveLoss(self.cfg.train.contrastive_loss_config.margin, use_cuda=self.cfg.use_cuda)
        if self.cfg.use_cuda:
            self.loss_MILcontr = self.loss_MILcontr.cuda()
    ...
    def compute_global_align_loss(self, visual_emb: th.Tensor, text_emb: th.Tensor) -> th.Tensor:
        """
    ...
    def compute_total_ce_loss(self, visual_data: model_retrieval.RetrievalVisualEmbTuple,
        ...
        losses = (self.loss_contr(visual_data.vid_emb, text_data.par_emb),
                  self.loss_MILcontr(visual_data.clip_emb, text_data.sent_emb))
    ...
    def train_model(self, train_loader: data.DataLoader, val_loader: data.DataLoader) -> None:
        ...
        for _epoch in range(self.state.current_epoch, self.cfg.train.num_epochs):
            ...
            for step, batch in enumerate(train_loader):  # type: RetrievalDataBatchTuple
                ...
                additional_log = f"L Contr: {contr_loss:.5f}"
